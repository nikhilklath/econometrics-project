...
use simdata_ah_diff_shock_ran_cons1.dta

hist y_a, frac width(10) xtitle("A's Y/L after 40 periods")
graph save y_a3,replace
hist y_b, frac width(10) xtitle("B's Y/L GDP after 40 periods")
graph save y_b3, replace
 
hist A_a,frac width(2) xtitle("A's productivity after 40 periods")
graph save A_a3,replace
hist A_b,frac width(2) xtitle("B's productivity after 40 periods")
graph save A_b3,replace

graph combine y_a3.gph y_b3.gph, row(1) title(`"A_A(0)<A_B(0), H_A(0)<H_B(0), ϵ_A=0.01, ϵ_B=abs(ϵ), ϵ~ N(0.01,0.005)"', size(medium))
graph save hist_y3.gph, replace
graph combine A_a3.gph A_b3.gph, row(1) title(`"A_A(0)<A_B(0), H_A(0)<H_B(0), ϵ_A=0.01, ϵ_B=abs(ϵ), ϵ~ N(0.01,0.005)"', size(medium))
graph save hist_A3.gph, replace

sum y_a y_b h_a h_b A_a A_b, sep(0)

forvalues i =12(12)50{
    clear all
    cd "C:\Users\nikhi\Desktop\BU_Sem_2\EC 508\Project\Data_ah_diff_shock_ran_cons1"
	use dataset`i'
	gen year =_n
...
graph combine y24.gph y_pc24.gph k_r24.gph l_r24.gph a_r24.gph, title(`"A_A(0)<A_B(0), H_A(0)<H_B(0), ϵ_A=0.01, ϵ_B=abs(ϵ), ϵ~ N(0.01,0.005)"', size(medium))
graph save hist_y3.gph, replace
