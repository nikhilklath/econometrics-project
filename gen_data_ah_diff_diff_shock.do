...
use simdata_ah_diff_diff_shock.dta
...
graph combine y_a2.gph y_b2.gph, row(1) title(`"A_A(0)<A_B(0), H_A(0)<H_B(0), ϵ_B≠ϵ_A=abs(ϵ), ϵ~ N(0.01,0.005)"', size(medium))
graph save hist_y2.gph, replace
graph combine A_a2.gph A_b2.gph, row(1) title(`"A_A(0)<A_B(0), H_A(0)<H_B(0), ϵ_B≠ϵ_A=abs(ϵ), ϵ~ N(0.01,0.005)"', size(medium))
graph save hist_A2.gph, replace

sum y_a y_b h_a h_b A_a A_b, sep(0)

forvalues i =12(12)50{
    clear all
    cd "C:\Users\nikhi\Desktop\BU_Sem_2\EC 508\Project\Data_ah_diff_diff_shock"
	use dataset`i'
	gen year =_n
...
graph combine y24.gph y_pc24.gph k_r24.gph l_r24.gph a_r24.gph, title(`"A_A(0)<A_B(0), H_A(0)<H_B(0), ϵ_B≠ϵ_A=abs(ϵ), ϵ~ N(0.01,0.005)"', size(medium))
