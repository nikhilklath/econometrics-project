...
use simdata_a_same_h_diff_shock_ran_cons1.dta

hist y_a, frac width(10) xtitle("A's Y/L after 40 periods")
graph save y_a,replace
hist y_b, frac width(10) xtitle("B's Y/L GDP after 40 periods")
graph save y_b, replace
 
hist A_a,frac width(2) xtitle("A's productivity after 40 periods")
graph save A_a,replace
hist A_b,frac width(2) xtitle("B's productivity after 40 periods")
graph save A_b,replace

graph combine y_a.gph y_b.gph A_a.gph A_b.gph, title(`"A_A(0)=A_B(0), H_A(0)<H_B(0), ϵ_A=0.01, ϵ_B=abs(ϵ), ϵ~ N(0.01,0.005)"', size(medium))
...
forvalues i =12(12)50{
    clear all
    cd "C:\Users\nikhi\Desktop\BU_Sem_2\EC 508\Project\Data_a_same_h_diff_shock_ran_cons1"
	use dataset`i'
	gen year =_n
